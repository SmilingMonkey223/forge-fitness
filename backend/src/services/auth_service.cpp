...
std::pair<User, JWT::TokenPair> AuthService::register_user(const RegisterRequest& req) {
    // Validate email
...
std::pair<User, JWT::TokenPair> AuthService::login(const LoginRequest& req) {
    auto conn = Database::instance().get_connection();
    pqxx::work txn(*conn);
...
JWT::TokenPair AuthService::refresh(const std::string& refresh_token) {
    // Verify token
